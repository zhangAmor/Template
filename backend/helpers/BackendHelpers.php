...
use backend\models\MenuExtends;
...
class BackendHelpers
{
     ...
     * @return array
     */
    public static function getAdminMenuDatas($roleid)
    {
        $menu_cache_datas = Yii::$app->mcache->get('menu_datas');

        //该角色的权限id
        $meueIds = RoleExtends::find()->where(['=','roleid',$roleid])->select('menu_ids')->one();
        $meueArr = explode(',',$meueIds['menu_ids']);
        ...
        foreach($menu_cache_datas as $menu_cache_data)
        {
            if($menu_cache_data['enabled'] == 1 && $menu_cache_data['is_show'] == 1)
            {
                ...
                {
                    if(in_array($menu_cache_data['menuid'],$meueArr))
                    {
                        $parent_datas[$menu_cache_data['parent_id']][] = $menu_cache_data;
                    }
                }
...
//    {
//        $menu = self::getMenuLevelDatas();
//        $list = self::demo($menu,0,0);
//        return $list;
//    }
     ...
     * @return array
     */
    public static function demo($arr,$id)
    {
        $list =array();
        foreach ($arr as $k=>$v){
            if ($v['parent_id'] == $id){
                $v['child'] = self::demo($arr,$v['menuid']);
                $list[] = $v;
            }
        }
        return $list;
