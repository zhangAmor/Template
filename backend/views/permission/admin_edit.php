...
<form class="layui-form" action="<?php echo Html::encode(Url::to(['permission/admin-edit', 'userid' => $admin_data['userid']])); ?>" onsubmit="return false;">
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <select name="AdminEditForm[roleid]">
                <option value="">请选择角色</option>
                <?php foreach($role_list as $v): ?>
                    <option value="<?php echo Html::encode($v['roleid']); ?>" <?php if($admin_data['roleid'] == $v['roleid']){echo 'selected';} ?>><?php echo Html::encode($v['name']); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
