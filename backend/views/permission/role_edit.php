...
<form class="layui-form" action="<?php echo Html::encode(Url::to(['permission/role-edit'])); ?>" onsubmit="return false;">
        ...
        <div class="layui-tab-content mos-common-padding0 mos-common-padding-top15">
            <div class="layui-tab-item layui-show">
                <div class="layui-form-item">
                    ...
                    <input type="hidden" name="RoleEditForm[roleid]" value="<?=$role_info->roleid?>">
                    <div class="layui-form-mid layui-word-aux"><?=$role_info->roleid?></div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">角色名</label>
                    <div class="layui-input-inline">
                        <input type="text" name="RoleEditForm[name]" lay-verify="required" lay-vertype="tips" placeholder="请输入角色名" class="layui-input" value="<?=$role_info->name?>">
                    </div>
                </div>
            </div>
                            ...
                            <fieldset class="layui-elem-field">
                                ...
                                <div class="layui-field-box">
                                    <div style="margin-left: <?=$v0['level']*40?>px;">
                                        <input class="mos-role-menu-input" type="checkbox" name="RoleEditForm[menuid][<?=$v0['menuid']?>]" value="<?=$v0['menuid']?>" title="<?=$v0['name']?>" lay-filter="role_menu" data-pid="<?=$v0['parent_id']?>" data-input-class="mos-role-menu-input" <?= in_array($v0['menuid'],$menu_arr)?'checked':'' ?>>
                                    ...
                                    <?php foreach ($v0['child'] as $v1){ ?>
                                        <div style="margin-left: <?=$v1['level']*40?>px;">
                                            <input class="mos-role-menu-input" type="checkbox" name="RoleEditForm[menuid][<?=$v1['menuid']?>]" value="<?=$v1['menuid']?>" title="<?=$v1['name']?>" lay-filter="role_menu" data-pid="<?=$v1['parent_id']?>" data-input-class="mos-role-menu-input" <?= in_array($v1['menuid'],$menu_arr)?'checked':'' ?>>
                                            <div class="layui-unselect layui-form-checkbox">
                                                <span><?=$v1['name']?></span>
                                                <i class="layui-icon layui-icon-ok"></i>
                                            </div>
                                        ...
                                        <?php foreach ($v1['child'] as $v2){ ?>
                                            <div style="margin-left: <?=$v2['level']*40?>px;">
                                                <input class="mos-role-menu-input" type="checkbox" name="RoleEditForm[menuid][<?=$v2['menuid']?>]" value="<?=$v2['menuid']?>" title="<?=$v2['name']?>" lay-filter="role_menu" data-pid="<?=$v2['parent_id']?>" data-input-class="mos-role-menu-input" <?= in_array($v2['menuid'],$menu_arr)?'checked':'' ?>>
                                                <div class="layui-unselect layui-form-checkbox">
                                                    <span><?=$v2['name']?></span>
                                                    <i class="layui-icon layui-icon-ok"></i>
                                                </div>
